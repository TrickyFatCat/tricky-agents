...
For rung 1, look in the tool list for tools whose names carry `lsp`,
`definition`, `references`, `hover` or `symbols`. Use them when present. Do
not name a specific server, because the available bridges change. Do not ask
